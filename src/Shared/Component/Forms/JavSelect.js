...
export default function JavSelect(props) {
    ...
    return (
        ...
        <div className={`flex bg-white dark:bg-[#242A38] items-center text-gray-600 dark:text-gray-200 ${props.bgColor}`}>
            {props.title && <span className={"text-xs pt-0 mx-1"}>{props.title}</span>}
            <div className={`relative ${props.hideBorder ? '' : 'border dark:border-gray-600'} w-full rounded px-1 py-2.5`}>
